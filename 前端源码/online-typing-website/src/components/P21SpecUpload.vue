<template>
  <div class="p21-spec-upload">
    <el-card class="upload-card" shadow="hover">
      <template #header>
        <div class="card-header">
          <span class="card-title">P21空SPEC上传器</span>
        ...
        <div class="upload-section">
          ...
          >
            ...
            <div class="el-upload__text">
              将P21空SPEC文件拖到此处，或<em>点击上传</em>
            </div>
            <template #tip>
              <div class="el-upload__tip">
                支持上传1个Excel格式文件(.xlsx, .xls)，文件大小不超过50MB
              </div>
            </template>
          </el-upload>
        </div>
              ...
              <template #default="scope">
                ...
                <div v-else class="parse-action">
                  <el-button 
                    ...
                    @click="processP21Spec(scope.row)"
                  >
          ...
          <el-collapse>
            <el-collapse-item title="使用说明" name="help">
              <div class="help-content">
                <h5>什么是P21空SPEC？</h5>
                <p>P21空SPEC是P21标准的空白规范模板，用于定义数据标准和结构，便于后续填充实际数据。</p>
                
                <h5>使用步骤：</h5>
                <ol>
                  <li>上传Excel格式的P21空SPEC文件</li>
...
const uploadUrl = `${baseUrl}/p21-spec/upload`
const uploadHeaders = {
...
const removeFile = async (index) => {
  ...
  try {
    ...
    // 调用后端删除API
    const response = await fetch(`${baseUrl}/p21-spec/delete?fileId=${encodeURIComponent(file.fileId)}`, {
      method: 'DELETE',
      headers: {
...
const handleReUpload = async (originalFile, newFile) => {
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/p21-spec/reupload`, {
      method: 'POST',
      headers: {
        'Accept': 'application/json'
      },
...
const processP21Spec = async (file) => {
  if (!file) {
  ...
  try {
    ElMessage.info('正在处理P21空SPEC文件，请稍候...')
    
    const response = await fetch(`${baseUrl}/p21-spec/process`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    if (result.success && result.data) {
      ...
      ElMessage.success('P21空SPEC文件处理成功!')
    } else {
      const errorResult = {
        success: false,
...
const formatUploadTime = (uploadTime) => {
  ...
  if (isNaN(time.getTime())) {
    console.warn('无效的上传时间:', uploadTime)
    return '-'
  }
...
const loadUploadRecords = async () => {
  try {
    const response = await fetch(`${baseUrl}/p21-spec/records`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    if (result.success && Array.isArray(result.data)) {
      ...
      console.log('已加载', result.data.length, '条P21空SPEC上传记录')
    } else {
