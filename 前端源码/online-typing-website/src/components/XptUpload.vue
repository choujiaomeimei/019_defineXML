<template>
  <div class="xpt-upload">
    <el-card class="upload-card" shadow="hover">
      <template #header>
        <div class="card-header">
          <span class="card-title">XPT文件上传器</span>
      ...
      <div class="upload-content">
        ...
        <div class="upload-section">
          ...
          >
            ...
            <div class="el-upload__text">
              将XPT文件拖到此处，或<em>点击上传</em>
            </div>
            <template #tip>
              <div class="el-upload__tip">
                支持上传多个XPT文件，每个文件大小不超过200MB，最多100个文件
              </div>
            </template>
          </el-upload>
        </div>
        ...
        <div v-else class="empty-state">
          ...
          <p class="empty-desc">请拖拽或点击上传XPT数据文件</p>
        </div>
    ...
    <el-dialog v-model="previewVisible" title="文件预览" width="60%">
      <div class="file-preview">
        <div class="preview-header">
          <h4>{{ currentPreviewFile?.originalName }}</h4>
          <p>文件大小: {{ formatFileSize(currentPreviewFile?.fileSize) }}</p>
        </div>
        <div class="preview-content">
          <el-alert
            title="XPT文件预览"
            description="XPT是SAS传输格式文件，包含数据集的结构和数据。文件内容需要专门的工具解析。"
            type="info"
            show-icon
            :closable="false"
          />
...
const uploadUrl = `${baseUrl}/xpt/upload`
const uploadHeaders = {
...
const beforeUpload = (file) => {
  const isXPT = file.type === 'application/octet-stream' || file.name.toLowerCase().endsWith('.xpt')
  if (!isXPT) {
    ElMessage.error('只能上传XPT格式文件!')
    return false
  }
...
const removeFile = async (index) => {
  ...
  try {
    ...
    // 调用后端删除API
    const response = await fetch(`${baseUrl}/xpt/delete?fileId=${encodeURIComponent(file.fileId)}`, {
      method: 'DELETE',
      headers: {
...
const handleReUpload = async (originalFile, newFile) => {
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/xpt/reupload`, {
      method: 'POST',
      headers: {
        'Accept': 'application/json'
      },
...
const processXpt = async (file) => {
  ...
  try {
    ElMessage.info('正在处理XPT文件，请稍候...')
    
    const response = await fetch(`${baseUrl}/xpt/process`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
const loadUploadRecords = async () => {
  try {
    const response = await fetch(`${baseUrl}/xpt/records`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json'
      }
    ...
    if (result.success && Array.isArray(result.data)) {
      ...
      console.log('已加载', result.data.length, '条XPT上传记录')
    } else {
