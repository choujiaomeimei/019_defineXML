...
class VLMCodeListDBManager:
    ...
    def __init__(self, db_config: Dict[str, str], project_id: str = "default"):
        """
        初始化数据库管理器
    ...
    def clear_project_data(self):
        ...
        try:
            ...
            # 清除CodeList数据
            cursor.execute("DELETE FROM sas_codelist_data WHERE project_id = %s", (self.project_id,))
            codelist_deleted = cursor.rowcount
            
            cursor.close()
            
            print(f"已清除项目 {self.project_id} 的旧数据: VLM {vlm_deleted} 条, CodeList {codelist_deleted} 条")
            return True
    ...
    def import_vlm_data(self, vlm_df: pd.DataFrame, created_by: str = "system"):
        ...
        try:
            ...
            INSERT INTO sas_vlm_data (
                project_id, dataset, variable, where_clause, label, 
                controlled_terms_or_format, origin, pages, derivation_comment, 
                method, comment, category, sort_order, created_by
            ) VALUES (
    ...
    def import_codelist_data(self, codelist_df: pd.DataFrame, created_by: str = "system"):
        ...
        try:
            ...
            INSERT INTO sas_codelist_data (
                project_id, vcd, vlabel, type, cdnum, code, 
